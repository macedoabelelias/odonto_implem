...
<td style="color:{$classe_ativo}" class="esc">{$executado}</td>
<td>
	<big><a class="btn btn-primary-light btn-sm" href="#" onclick="editar('{$id}','{$nome}','{$valor}','{$tempo}','{$exame}','{$convenio}','{$preparo}','{$dias_retorno}')" title="Editar Dados"><i class="fa fa-edit "></i></a></big>
...
<script type="text/javascript">
	function editar(id, nome, valor, tempo, exame, convenio, preparo, dias_retorno){
		$('#mensagem').text('');
