...
if($nivel_usuario == 'Administrador'){
	$query = $pdo->query("SELECT * from $tabela where nivel != 'Cliente' and acessar_painel != 'Não' order by id desc");
}else{
	$query = $pdo->query("SELECT * from $tabela where nivel != 'Cliente' and nivel != 'Administrador' and id != $id_usuario and acessar_painel != 'Não' order by id desc");
}
