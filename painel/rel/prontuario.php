		...
		<table style="width: 100%; border: 0px solid #ccc;">
			...
			<tr>
				...
				<td style="width: 55%; text-align: right; font-size: 9px;padding-right: 10px;">
						...
						<b><big>PRONTUÁRIO DO PACIENTE</big></b><br>
...
<hr style="margin-bottom: 0;">
	...
	<table style="width:100%;">
		...
		<tr style="width:100%;">
			...
			<td style="width:60%; font-size: 10px; text-align: left;"><?php echo $nome_sistema ?> / Telefone: <?php echo $telefone_sistema ?> / Email: <?php echo $email_sistema ?> / Este documento é confidencial!</td>
...
if($total_itens == 0){
	...
	echo '<span style="font-size:13px">Nenhuma Anamnese Lançada!</span>';
		...
		<?php 
			...
			$query = $pdo->query("SELECT * FROM historico_paciente where paciente = '$id' order by id desc");
...
$total_reg = @count($res);
		...
		<div align="left" style="margin-top: 20px; margin-bottom: 10px; border-bottom: 1px solid #000; font-size:12px"><b>HISTÓRICO CLÍNICO / EVOLUÇÃO</b></div>
...
<div style="margin-top: 25px; margin-bottom: 10px; border-bottom: 1px solid #000; font-size:12px"><b>ÚLTIMOS PROCEDIMENTOS REALIZADOS</b></div>
...
<table id="cabecalhotabela" style="border-bottom-style: solid; font-size: 11px; margin-bottom:10px; width: 100%; table-layout: fixed;">
			...
			<thead>			
				...
				<tr id="cabeca" style="margin-left: 0px; background-color:#CCC">
					<td style="width:40%">PROCEDIMENTO</td>
					<td style="width:40%">PROFISSIONAL</td>				
...
$total_soma = 0;
$query = $pdo->query("SELECT * from agendamentos where status LIKE 'Finalizado' and paciente = '$id_paciente' order by data desc, hora desc");
...
<td style="width:40%"><?php echo $nome_serv ?></td>
<td style="width:40%"><?php echo $nome_funcionario ?></td>
